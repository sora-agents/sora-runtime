...
``result_sink``. The three sink tests are promoted verbatim from the walking-skeleton spike
(``tests/test_cycle_wiring.py``); ``drain_snapshots_current_depth`` in particular pins the
within-cycle no-starvation guarantee (an item pushed *during* a drain waits for the next drain). The
Observe phase is the sink's first real consumer, so the primitive lands its permanent home here.
"""
