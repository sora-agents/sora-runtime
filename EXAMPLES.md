...
The `$decide` above bundles two conditions — *"violent-crime index in 5..10 **and** not already saved"*. The "not already saved" half no longer needs the model: a mechanical `not_in` against another collection expresses it deterministically — `{"path": "apartment_id", "op": "not_in", "value": {"$from": "list_saved_apartments"}, "value_path": "apartment_id"}` keeps only the candidates whose id is absent from the saved list. And with `{"all": [...]}` composition the two halves no longer have to travel together: `{"all": [{"path": "crime_index", "op": "between", "value": [5, 10]}, {"path": "apartment_id", "op": "not_in", "value": {"$from": "list_saved_apartments"}, "value_path": "apartment_id"}]}` is the whole predicate, mechanically. That matters more than it looks — before composition, one clause needing judgement dragged the *entire* predicate onto a model call over every item. A sibling `overlaps` op does the same for ranges (one element's `start_path`/`end_path` against a whole collection of intervals, half-open unless `"boundaries": "inclusive"`), so a clash rule needs no `$decide` either. Cross-collection membership (`in`/`not_in` with a reference `value`, projected by `value_path`), `all`/`any` composition and `overlaps` all resolve their operands *once in Reason*, keeping the predicate itself a literal comparison; they are [ADR-0023](docs/architecture/adrs/0023-structured-value-data-ops.md) extensions, with the cost measurement and the rejected general-quantifier alternative in [the design note](docs/architecture/notes/mechanical-predicates-over-model-filters.md).

Both of those narrowings assume the collection is already in hand — here it is, because the ARE adapter publishes each app's whole state as an observable property, so `{"$prop": "Contacts.state"}` *is* the 125 contacts. Where an environment publishes no observable state, the same collection is reachable only a page at a time, and the plan has to assemble it before it can filter: fan the list operation out over a **literal** list of offsets with a mechanical sub-goal (`"in": [0, 20, 40, …]`, `"as": "offset"`), `collect` those runs, and `flatten` the pages into records. `flatten` is the step that makes this work rather than silently fail — `collect` yields one item per *call*, so without it the filter tests pages, matches nothing, and the agent reports that no such record exists. The sweep's *bound* is still the plan author's to choose: an offset past the end returns an empty page and costs one call, so sweeping long is cheap and stopping short drops records silently.
...
Observe's existing resume pass matches the gate and returns the activity to `READY` — mechanically, no model. Being eligible is not the same as holding, so Reason then spends **one** batched `_infer_` over every eligible condition (*which `when`s hold, is any `until` satisfied, and if so the plan for its `then`*), and pushes `then` as a frame. One extra model call for the whole scenario, against the seven the failing run already spent. The change that fired the condition is not thrown away when the frame is pushed: the runtime seeds `fired_added_ids` / `fired_removed_ids` / `fired_updated_ids` as reserved bindings (the only ones that survive a replan, and the only ones rendered into the plan prompt), so the sub-plan can write `{"$bind": "fired_added_ids"}` instead of paying a model call to re-derive the set the signal already reported verbatim.

That batched call is woken by a change, though, so it never runs for a condition nothing ever moves against — and a watch that goes quiet would hold its activity `BLOCKED` for good. So Observe also runs a **retire-only sweep** over quiet conditions ([ADR-0027](docs/architecture/adrs/0027-achievement-and-maintenance-goals.md)), on the same idle cadence as layer 3 below: one activity at a time, never on a tick where something could advance, with exponential backoff because an activity blocked on a condition makes *every* tick idle. It can only retire, never fire — a firing pass would duplicate the gate's job on no evidence at all, since the absence of a change is not evidence that one happened. Observe drops what retired and releases the wait; the pop that follows is Reason's, which is what finally lets a maintenance frame's parent resume.
