...
| `context_adaptation` | `before_writes` | A *level name* (`none` \| `before_writes` \| `before_each_op` \| `replan_on_change`) or a dotted path to a custom `ReconsiderationPolicy`. `replan_on_change` gates where `before_writes` does but discards the plan instead of spending a revalidation call — for an environment whose percepts announce that the world moved without saying to what, where a judge shown only the announcement cannot do better than the gate. Unsound where an adapter's signals include the agent's own writes. Not a dotted-path-only key like the others — see [Extension Protocols — Auxiliary seams](extension-protocols.md#auxiliary-seams). |
